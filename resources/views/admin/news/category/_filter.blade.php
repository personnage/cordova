...
<ul class="nav nav-pills">
  ...
  <a href="{{ url('admin/news/category') }}">Active <span class="badge">{{ $active }}</span></a>
  </li>

  <li role="presentation" @if(request('filter')=='deleted') class="active" @endif>
    <a href="{{ url('admin/news/category').'?filter=deleted' }}">Deleted <span class="badge">{{ $deleted }}</span></a>
  </li>
</ul>
